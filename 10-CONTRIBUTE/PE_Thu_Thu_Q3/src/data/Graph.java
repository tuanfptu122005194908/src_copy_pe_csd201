...
//===========================================================================
    ...
    display S vertices from the 2nd vertex to the 6th vertex only. Hint: copy depth(...) to depth2(...)
...
//=================================================================
    ...
    1 contains vertices in shortest path (2), line 2 contains the last 4 vertices selected into the set S,
    line 3 contains vertices in shortest path (1). (Note that in the weighted matrix, the value 99 is
    considered as infinity). Output in the file f2.txt must be the following:
    A B C E D G
    E D F G
    C E D F
    */
    ...
    void dijkstra2(int fro, int to, RandomAccessFile f) throws IOException {
        ...
        i = s.pop();
        int mm = listSelected.size() - 4; // Sua so 4
        f.writeBytes("" + v[listSelected.get(mm)]);
        for (int m = listSelected.size() - 3; m < listSelected.size(); m++) { // Sua so 3
            int index = listSelected.get(m);
            f.writeBytes(" " + v[index]);
        }
        f.writeBytes("\r\n");
    }
